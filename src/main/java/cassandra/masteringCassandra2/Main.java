...
import static cassandra.masteringCassandra2.RandomGenerator.getCommenter;
import static cassandra.masteringCassandra2.RandomGenerator.getInt;
import static cassandra.masteringCassandra2.RandomGenerator.getString;
import static cassandra.masteringCassandra2.RandomGenerator.getTag;
import static cassandra.masteringCassandra2.RandomGenerator.getTimeUUID;
import static cassandra.masteringCassandra2.RandomGenerator.getUUID;
import cassandra.masteringCassandra2.CassandraConnection.SessionWrapper;
import cassandra.masteringCassandra2.exceptions.BlogNotFoundException;
...
//			post.setPostedOn(System.currentTimeMillis());
			post.setPostedOn(Date.from(LocalDate.now().atStartOfDay().toInstant(ZoneOffset.UTC)));
...
//			com.setPostedOn(System.currentTimeMillis());
			com.setPostedOn(Date.from(LocalDate.now().atStartOfDay().toInstant(ZoneOffset.UTC)));
...
//								+ new Date(comment.getPostedOn()).toString());
	...
	public static void run() {
		try {
			...
			log.error("Something went wrong, please look into the stacktrace here:", e);
			System.exit(1);
		} finally {
			cleanup();
		}
